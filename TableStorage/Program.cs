...
namespace TableStorage
{
    public class Program
    {
        ...
        {
            var order = new OrderEntity("Wolfgang", "20180404")
            {
                OrderNumber = "123",
                ShippedDate = Convert.ToDateTime("2018/04/06"),
                Status = "shipped"
            };
        ...
        private static void GetAllRecords(CloudTable table)
        {
            var query = new TableQuery<OrderEntity>().Where(
                TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, "Wolfgang"));
        ...
        {
            var updateEntity = RetrieveEntity("Wolfgang", "20180405", table);
        ...
        {
            var deleteEntity = RetrieveEntity("Wolfgang", "20180405", table);
